...
public class AshDustItem extends Item {
    ...
    public TypedActionResult<ItemStack> use( World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            AshDustProjectileEntity ashDustProjectileEntity = new AshDustProjectileEntity(user, world);
            ashDustProjectileEntity.setItem(itemStack);
            ashDustProjectileEntity.setVelocity(user, user.getPitch(), user.getYaw(), 0.0F, 1.5F, 1.0F);
            world.spawnEntity(ashDustProjectileEntity);
        }
