...
public class InfernoEssenceTrunkPlacer extends TrunkPlacer {
    public static final Codec<InfernoEssenceTrunkPlacer> CODEC  = RecordCodecBuilder.create(infernoEssenceTrunkPlacerInstance ->
            fillTrunkPlacerFields(infernoEssenceTrunkPlacerInstance).apply(infernoEssenceTrunkPlacerInstance, InfernoEssenceTrunkPlacer::new));

    public InfernoEssenceTrunkPlacer( int baseHeight, int firstRandomHeight, int secondRandomHeight ) {
        super(baseHeight, firstRandomHeight, secondRandomHeight);
    }

    @Override
    protected TrunkPlacerType<?> getType() {
        return ModTrunkPlacerTypes.INFERNO_ESSENCE_TRUNK_PLACER;
    }
    ...
    public List<FoliagePlacer.TreeNode> generate( TestableWorld world, BiConsumer<BlockPos, BlockState> replacer,
        ...
        int height_ = height + random.nextBetween(firstRandomHeight, firstRandomHeight + 3) + random.nextBetween(secondRandomHeight - 1, secondRandomHeight + 1);
